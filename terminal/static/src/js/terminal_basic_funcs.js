        ...
        _searchModelRecord: function (params) {
            ...
            return rpc.query({
                ...
                for (var index in result) {
                    self.print(_.template("<span class='o_terminal_click " +
                        "o_terminal_view' data-resid='<%= id %>' " +
                        "data-model='<%= model %>'><%= id %></span>. ")({
                        id:result[index].id,
                        model:model,
                    }), true);
                    ...
                    if (ControllerSelectedIds.length) {
                        ...
                        ds.call('get_metadata', [self._view_manager.active_view.controller.getSelectedIds()]).done(function (result) {
                            ...
                            metadata.create_date = field_utils.format.datetime(
                                createDate);
                            var modificationDate = field_utils.parse.datetime(
                                metadata.write_date);
                            ...
                            self.print(_.template("<span style='color: " +
                                "gray;'>Creation Date:</span> <%= date %>")(
                                {date: metadata.create_date}));
                            self.print(_.template("<span style='color: "+
                                "gray;'>Last Modification By:</span> " +
                                "<span class='o_terminal_click " +
                                "o_terminal_cmd' data-cmd='view res.users " +
                                "<%= uid %>'><%= user %></span>")({
                                user: metadata.lastModifiedBy,
                                uid: metadata.write_uid[0],
                            }));
                            self.print(_.template("<span style='color: " +
                                "gray;'>Last Modification Date:</span> " +
                                "<%= date %>")({date: metadata.write_date}));
                        });
                    } else {
